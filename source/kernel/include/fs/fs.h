...
typedef struct _fs_t {
    char mount_point[FS_MOUNTP_SIZE];       // mount point
    fs_type_t type;              // file system type

    fs_op_t * op;              // file system op interface
    void * data;                
    int dev_id;                 // associated device

    list_node_t node;           // next node

    // This is currently designed this way, which may not look optimal, but it's the simplest method for now. 
    // This way, we don't have to worry about memory allocation issues.
    union {
        fat_t fat_data;         // file system related data
    };
    mutex_t * mutex;              // mutex
}fs_t;
